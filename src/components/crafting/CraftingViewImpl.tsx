...
interface CraftingViewProps {
  state: GameState;
  /** Existing app-level reducer gateway. Also carries encoded deep-production commands. */
  ...
  onQueueMaintenance?: (targetInstanceId: string, mode: MaintenanceMode, componentInstanceId?: string, survivorId?: string) => void;
  ...
  onQueueComponentModification?: (instanceId: string, modification: ComponentModification, componentInstanceId?: string, survivorId?: string) => void;
...
export const CraftingView: React.FC<CraftingViewProps> = ({
  ...
  const fallbackWorkerId = (preferred?: string) =>
    preferred
    || selectedSurvivorId
    || survivors.find((survivor) => survivor.currentAction.type === 'idle')?.id
    || survivors[0]?.id
    || '';

  const dispatchProductionCommand = (command: string, preferredSurvivorId?: string) => {
    const workerId = fallbackWorkerId(preferredSurvivorId);
    if (!workerId || !onStartCrafting) return;
    onStartCrafting(workerId, command);
  };
  ...
  const getRecipeStatus = (recipe: RecipeDefinition) => {
    ...
    recipe.ingredients.forEach((ingredient) => {
      ...
      maxCraft = Math.min(maxCraft, Math.floor(available / ingredient.quantity));
      if (available < ingredient.quantity) missingIngredients.push(ingredient.itemId);
    });
    ...
    <div className="crafting-reference-root w-full h-full min-h-0 flex flex-col overflow-hidden bg-[#06130f] text-[#e8e2d4] select-none" style={{ fontFamily: UI_FONT, containerType: 'size' }}>
      ...
      {activeTab === 'craft' && activeRecipe && (
        <div className="min-h-0 flex-1 grid grid-cols-[minmax(0,2.55fr)_minmax(300px,.95fr)] gap-2 p-2 overflow-hidden">
          <div className="min-w-0 min-h-0 flex flex-col border border-[#40503d] bg-[#071711] overflow-hidden">
            <CraftingHeader category={selectedCategory} onSelectCategory={setSelectedCategory} categoryCounts={categoryCounts} searchQuery={searchQuery} onSearchChange={setSearchQuery} sortMode={sortMode} onSortChange={setSortMode} viewMode={viewMode} onToggleViewMode={() => setViewMode((previous) => previous === 'grid' ? 'list' : 'grid')} onlyPinned={onlyPinned} onToggleOnlyPinned={() => setOnlyPinned((previous) => !previous)} stats={stats} />
            <div className="min-h-0 flex-1 grid grid-cols-[1.08fr_.92fr] gap-2 p-2 overflow-hidden">
              <section className="min-w-0 min-h-0 border border-[#354a3b] bg-[#081813] p-2 flex flex-col overflow-hidden">
                ...
                <div className="min-h-0 flex-1"><CraftingCatalogGrid recipes={processedRecipes} selectedRecipeId={activeRecipe.id} onSelectRecipe={setSelectedRecipeId} onTogglePin={togglePin} viewMode={viewMode} /></div>
              </section>
              <CraftingDetailCard recipe={activeRecipe} inventoryItems={inventory.items} survivors={survivors} selectedSurvivorId={selectedSurvivorId} onSelectSurvivor={setSelectedSurvivorId} onCraftNow={handleCraftNow} onAddToQueue={handleAddToQueue} isPinned={pinnedIds.has(activeRecipe.id)} onTogglePin={togglePin} />
            </div>
          ...
          <CraftingQueueSidebar queue={craftingQueue} survivors={survivors} selectedRecipe={activeRecipe} onSelectRecipe={setSelectedRecipeId} onCancelQueueItem={onCancelQueueItem} onTogglePauseQueueItem={onTogglePauseQueueItem} onReorderQueue={onReorderQueue} stats={stats} onAddSelectedToQueue={() => handleAddToQueue(activeRecipe.id, 1, selectedSurvivorId)} />
        </div>
      )}
      ...
      {activeTab === 'research' && (
        <div className="min-h-0 flex-1 p-2 overflow-hidden">
          <ResearchView
            ...
            onAnalyzeResearch={(recipeId, survivorId) => {
              if (onAnalyzeResearch) onAnalyzeResearch(recipeId, survivorId);
              else dispatchProductionCommand(`__research_analyze__:${recipeId}`, survivorId);
            }}
            onToggleTrackResearch={(recipeId) => {
              if (onToggleTrackResearch) onToggleTrackResearch(recipeId);
              else dispatchProductionCommand(`__research_track__:${recipeId}`);
            }}
          />
        </div>
      )}
      ...
      {activeTab === 'repair' && (
        <div className="min-h-0 flex-1 p-2 overflow-hidden">
          <RepairView
            state={state}
            onQueueMaintenance={(instanceId, mode, componentId, survivorId) => {
              if (onQueueMaintenance) onQueueMaintenance(instanceId, mode, componentId, survivorId);
              else dispatchProductionCommand(`__maintenance__:${mode}:${instanceId}:${componentId || ''}`, survivorId);
            }}
            onCancelMaintenance={(jobId) => {
              if (onCancelMaintenance) onCancelMaintenance(jobId);
              else dispatchProductionCommand(`__maintenance_cancel__:${jobId}`);
            }}
            onTogglePauseMaintenance={(jobId) => {
              if (onTogglePauseMaintenance) onTogglePauseMaintenance(jobId);
              else dispatchProductionCommand(`__maintenance_pause__:${jobId}`);
            }}
            onDismantleTool={(instanceId, survivorId) => {
              if (onDismantleTool) onDismantleTool(instanceId, survivorId);
              else dispatchProductionCommand(`__dismantle__:${instanceId}`, survivorId);
            }}
          />
        </div>
      )}
      ...
      {activeTab === 'upgrade' && (
        <div className="min-h-0 flex-1 p-2 overflow-hidden">
          <UpgradeView
            state={state}
            onQueueTierUpgrade={(instanceId, survivorId) => {
              if (onQueueTierUpgrade) onQueueTierUpgrade(instanceId, survivorId);
              else dispatchProductionCommand(`__upgrade_tier__:${instanceId}`, survivorId);
            }}
            onQueueComponentModification={(instanceId, mod, componentId, survivorId) => {
              if (onQueueComponentModification) onQueueComponentModification(instanceId, mod, componentId, survivorId);
              else dispatchProductionCommand(`__upgrade_mod__:${mod}:${instanceId}:${componentId || ''}`, survivorId);
            }}
            onCancelUpgrade={(jobId) => {
              if (onCancelUpgrade) onCancelUpgrade(jobId);
              else dispatchProductionCommand(`__upgrade_cancel__:${jobId}`);
            }}
            onTogglePauseUpgrade={(jobId) => {
              if (onTogglePauseUpgrade) onTogglePauseUpgrade(jobId);
              else dispatchProductionCommand(`__upgrade_pause__:${jobId}`);
            }}
          />
        </div>
      )}
