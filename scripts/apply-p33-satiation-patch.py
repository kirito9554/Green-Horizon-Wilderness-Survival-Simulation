...
old = '''  const reserveCapacity = Math.max(0.001, population.maxEnergyReserveKg || 0.001);
...
new = '''  const tickDemandKg = Math.max(0.001, dailyDemandKg * elapsedDays);
...
if old not in text:
    raise SystemExit('hunt-drive block not found')
text = text.replace(old, new, 1)

old_loop = '''  for (let attemptIndex = 0; attemptIndex < attempts; attemptIndex += 1) {
    const ordinal = telemetry.attempts + 1;
'''
new_loop = '''  for (let attemptIndex = 0; attemptIndex < attempts; attemptIndex += 1) {
...
if old_loop not in text:
    raise SystemExit('hunt loop block not found')
