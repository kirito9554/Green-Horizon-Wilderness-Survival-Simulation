...
function assertRuntimeInvariants(seed: string, days: number): {
  population: number;
  births: number;
  deaths: number;
  moved: number;
  ...
  assert.ok(population >= initialPopulation * .55, `${seed}: first-pass demography should not collapse the entire fauna community (${initialPopulation} -> ${population})`);
  ...
  for (const speciesState of runtime.species) {
    ...
    for (const cohort of speciesState.cohorts) {
      ...
      assert.ok(allocationByPatch.has(cohort.patchId), `${seed}: dispersal entered unsupported habitat for ${speciesState.speciesId}`);
  ...
  console.log(`[${seed}] day=${days} population=${population}/${world.faunaCommunity.totalCarryingCapacity} births=${births} deaths=${deaths} moved=${moved} crossRegion=${crossRegionMoved} condition=${runtime.telemetry.meanCondition.toFixed(3)}`);
  return { population, births, deaths, moved, crossRegionMoved, signature };
}
